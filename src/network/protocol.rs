...
pub fn read_length_prefixed<R: Read>(reader: &mut R) -> Result<Vec<u8>, StrangecoinError> {
    ...
    reader.read_exact(&mut len_buf)
        .map_err(|_| StrangecoinError::IoError(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "length prefix")))?;
    ...
    reader.read_exact(&mut buf)
        .map_err(|_| StrangecoinError::IoError(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "message body")))?;
    Ok(buf)
}